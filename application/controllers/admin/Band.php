...
class Band extends CI_Controller {
	...
	public function index(){	
		$data['band_list'] = $this->band_model->get_all_data();
		$data['view'] = 'admin/band/band_list';
	...
	public function edit($id = 0){
		if($this->input->post('submit')){
			...
			$data['band_thumbnail_chk'] = $this->common_model->get_data_by_id($where,'listings');

			//************ thumbnail **************
			if(empty($_FILES['thumbnail']['name'])){
				$thumbnail_path = $data['band_thumbnail_chk']['thumbnail'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['thumbnail']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$thumbnail_path = $this->do_upload($_FILES['thumbnail']['name'],"thumbnail","band");
			}
			//**********************

			//************ feature image **************
			if(empty($_FILES['feature_image1']['name'])){
				$feature_image1 = $data['band_thumbnail_chk']['feature_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['feature_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$feature_image1 = $this->do_upload($_FILES['feature_image1']['name'],"feature_image1","band");
			}

			if(empty($_FILES['feature_image2']['name'])){
				$feature_image2 = $data['band_thumbnail_chk']['feature_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['feature_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$feature_image2 = $this->do_upload($_FILES['feature_image2']['name'],"feature_image2","band");
			}

			if(empty($_FILES['feature_image3']['name'])){
				$feature_image3 = $data['band_thumbnail_chk']['feature_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['feature_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$feature_image3 = $this->do_upload($_FILES['feature_image3']['name'],"feature_image3","band");
			}
			//**********************

			//************ exterior image **************
			if(empty($_FILES['exterior_image1']['name'])){
				$exterior_image1 = $data['band_thumbnail_chk']['exterior_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['exterior_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$exterior_image1 = $this->do_upload($_FILES['exterior_image1']['name'],"exterior_image1","band");
			}

			if(empty($_FILES['exterior_image2']['name'])){
				$exterior_image2 = $data['band_thumbnail_chk']['exterior_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['exterior_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$exterior_image2 = $this->do_upload($_FILES['exterior_image2']['name'],"exterior_image2","band");
			}

			if(empty($_FILES['exterior_image3']['name'])){
				$exterior_image3 = $data['band_thumbnail_chk']['exterior_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['exterior_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$exterior_image3 = $this->do_upload($_FILES['exterior_image3']['name'],"exterior_image3","band");
			}
			//**********************

			//************ interior image **************
			if(empty($_FILES['interior_image1']['name'])){
				$interior_image1 = $data['band_thumbnail_chk']['interior_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['interior_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$interior_image1 = $this->do_upload($_FILES['interior_image1']['name'],"interior_image1","band");
			}

			if(empty($_FILES['interior_image2']['name'])){
				$interior_image2 = $data['band_thumbnail_chk']['interior_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['interior_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$interior_image2 = $this->do_upload($_FILES['interior_image2']['name'],"interior_image2","band");
			}

			if(empty($_FILES['interior_image3']['name'])){
				$interior_image3 = $data['band_thumbnail_chk']['interior_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['band_thumbnail_chk']['interior_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/band/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/band/'. $exist_image_name);
            	}
                //===============
				$interior_image3 = $this->do_upload($_FILES['interior_image3']['name'],"interior_image3","band");
			}
			//**********************															
			$mon_start = $this->input->post('mon_start');
			$mon_end = $this->input->post('mon_end');
			...
			$data['all_users'] = $this->common_model->get_all_data('users'); 
			//***********************
			$data['details'] = $this->band_model->get_data_by_id($id);
			$data['view'] = 'admin/band/band_edit';
			$this->load->view('admin_layout', $data);
		}
	} 
		   
	public function do_upload($image,$post_image_field_name,$upload_folder_name){ 
        $config['upload_path']          = './uploads/band/';
	...
	public function del($id = 0){
		...
		if(!empty($data['listings_image_chk']['thumbnail'])){
            ...
            if (file_exists(FCPATH .'/uploads/band/'. $exist_del_image_name)) {
            	unlink(FCPATH .'/uploads/band/'. $exist_del_image_name);
            }				
		}
	...
	public function view($id = 0){
        $data['details'] = $this->band_model->get_data_by_id($id);
		$data['view'] = 'admin/band/band_view';
		$this->load->view('admin_layout', $data);
	}	
