...
class Choir extends CI_Controller {
	...
	public function index(){	
		$data['choir_list'] = $this->choir_model->get_all_data();
		$data['view'] = 'admin/choir/choir_list';
	...
	public function add(){
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					//***********************
					$data['view'] = 'admin/choir/choir_add';
					$this->load->view('admin_layout', $data);
				}else{
					//************ thumbnail **************
						if(empty($_FILES['thumbnail']['name'])){
	...
	public function edit($id = 0){
		if($this->input->post('submit')){
			...
			$data['choir_thumbnail_chk'] = $this->common_model->get_data_by_id($where,'listings');

			//************ thumbnail **************
			if(empty($_FILES['thumbnail']['name'])){
				$thumbnail_path = $data['choir_thumbnail_chk']['thumbnail'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['thumbnail']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$thumbnail_path = $this->do_upload($_FILES['thumbnail']['name'],"thumbnail","choir");
			}
			//**********************

			//************ feature image **************
			if(empty($_FILES['feature_image1']['name'])){
				$feature_image1 = $data['choir_thumbnail_chk']['feature_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['feature_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$feature_image1 = $this->do_upload($_FILES['feature_image1']['name'],"feature_image1","choir");
			}

			if(empty($_FILES['feature_image2']['name'])){
				$feature_image2 = $data['choir_thumbnail_chk']['feature_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['feature_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$feature_image2 = $this->do_upload($_FILES['feature_image2']['name'],"feature_image2","choir");
			}

			if(empty($_FILES['feature_image3']['name'])){
				$feature_image3 = $data['choir_thumbnail_chk']['feature_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['feature_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$feature_image3 = $this->do_upload($_FILES['feature_image3']['name'],"feature_image3","choir");
			}
			//**********************

			//************ exterior image **************
			if(empty($_FILES['exterior_image1']['name'])){
				$exterior_image1 = $data['choir_thumbnail_chk']['exterior_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['exterior_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$exterior_image1 = $this->do_upload($_FILES['exterior_image1']['name'],"exterior_image1","choir");
			}

			if(empty($_FILES['exterior_image2']['name'])){
				$exterior_image2 = $data['choir_thumbnail_chk']['exterior_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['exterior_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$exterior_image2 = $this->do_upload($_FILES['exterior_image2']['name'],"exterior_image2","choir");
			}

			if(empty($_FILES['exterior_image3']['name'])){
				$exterior_image3 = $data['choir_thumbnail_chk']['exterior_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['exterior_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$exterior_image3 = $this->do_upload($_FILES['exterior_image3']['name'],"exterior_image3","choir");
			}
			//**********************

			//************ interior image **************
			if(empty($_FILES['interior_image1']['name'])){
				$interior_image1 = $data['choir_thumbnail_chk']['interior_image1'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['interior_image1']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$interior_image1 = $this->do_upload($_FILES['interior_image1']['name'],"interior_image1","choir");
			}

			if(empty($_FILES['interior_image2']['name'])){
				$interior_image2 = $data['choir_thumbnail_chk']['interior_image2'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['interior_image2']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$interior_image2 = $this->do_upload($_FILES['interior_image2']['name'],"interior_image2","choir");
			}

			if(empty($_FILES['interior_image3']['name'])){
				$interior_image3 = $data['choir_thumbnail_chk']['interior_image3'];
			}else{
				// exploding https path and converting folder path for unlink image
                $exp_image = explode('/',$data['choir_thumbnail_chk']['interior_image3']);
                $exist_image_name = end($exp_image);
                if (file_exists(FCPATH .'/uploads/choir/'. $exist_image_name)) {
                	unlink(FCPATH .'/uploads/choir/'. $exist_image_name);
            	}
                //===============
				$interior_image3 = $this->do_upload($_FILES['interior_image3']['name'],"interior_image3","choir");
			}
			//**********************															
			$mon_start = $this->input->post('mon_start');
			$mon_end = $this->input->post('mon_end');
			...
			$data = array(
				...
				'youtube_url' => $this->input->post('youtube_url'),
				'faq_details' => $this->input->post('faq_details'),
				'choir_dope_vocalist' => $this->input->post('choir_dope_vocalist'),
				'choir_nice_backups' => $this->input->post('choir_nice_backups'),
				'choir_keyboardist' => $this->input->post('choir_keyboardist'),
				'choir_bassist' => $this->input->post('choir_bassist'),
				'choir_talking_drummer' => $this->input->post('choir_talking_drummer'),
				'choir_dope_music' => $this->input->post('choir_dope_music'),
				'choir_saxophonist' => $this->input->post('choir_saxophonist'),
				'choir_drummer' => $this->input->post('choir_drummer'),
				'choir_guitarist' => $this->input->post('choir_guitarist'),
				'feature_image1' => $feature_image1,
				'feature_image2' => $feature_image2,
				'feature_image3' => $feature_image3,
				'exterior_image1' => $exterior_image1,
			...
			$data['all_users'] = $this->common_model->get_all_data('users'); 
			//***********************
			$data['details'] = $this->choir_model->get_data_by_id($id);
			$data['view'] = 'admin/choir/choir_edit';
			$this->load->view('admin_layout', $data);
		}
	} 
		   
	public function do_upload($image,$post_image_field_name,$upload_folder_name){ 
        $config['upload_path']          = './uploads/choir/';
	...
	public function del($id = 0){
		...
		if(!empty($data['listings_image_chk']['thumbnail'])){
            ...
            if (file_exists(FCPATH .'/uploads/choir/'. $exist_del_image_name)) {
            	unlink(FCPATH .'/uploads/choir/'. $exist_del_image_name);
            }				
		}
	...
	public function view($id = 0){
        $data['details'] = $this->choir_model->get_data_by_id($id);
		$data['view'] = 'admin/choir/choir_view';
		$this->load->view('admin_layout', $data);
	}	
